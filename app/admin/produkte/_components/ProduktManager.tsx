...
function ProduktForm({ initial, onSave, onCancel, loading, error, submitLabel }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-semibold text-gray-500 block mb-1.5">Produktname *</label>
          <input type="text" placeholder="z.B. Buchungstool für Handwerker" value={form.name}
        ...
        <div>
          <label className="text-xs font-semibold text-gray-500 block mb-1.5">Preis <span className="font-normal text-gray-400">(optional, frei als Text)</span></label>
          <input type="text" placeholder="z.B. 29 €/Mo · kostenlos · ab 49 €" value={form.price}
      ...
      <div>
        ...
        <textarea placeholder="Was macht das Produkt, für wen ist es?" value={form.description}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" placeholder="z.B. SaaS, Handwerk" value={form.tags}
            onChange={e => set('tags', e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand" />
        </div>
        <div>
          <label className="text-xs font-semibold text-gray-500 block mb-1.5">URL <span className="font-normal text-gray-400">(optional)</span></label>
          <input type="url" placeholder="https://produkt.klanmedia.de" value={form.url}
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" checked={form.is_available} onChange={e => set('is_available', e.target.checked)} className="accent-blue-600" />
        <span className="text-sm text-gray-600">Bereits verfügbar (sonst "Coming soon")</span>
      </label>
...
export default function ProduktManager({ initialProdukte }: { initialProdukte: Produkt[] }) {
  const router = useRouter()
  const [, startTransition] = useTransition()
  const [items, setItems] = useState<Produkt[]>(initialProdukte)
  ...
  async function handleToggle(item: Produkt, field: 'is_visible' | 'is_available') {
  ...
  async function handleEdit(id: string, formData: typeof EMPTY_FORM) {
    ...
    const res = await fetch('/api/admin/produkte', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, name: formData.name, description: formData.description, icon: formData.icon, color: formData.color, tags, url: formData.url || null, price: formData.price || null, is_available: formData.is_available }),
    })
    const json = await res.json()
    setEditLoading(false)
    if (!res.ok) { setEditError(json.error ?? 'Fehler.'); return }
    setItems(prev => prev.map(p => p.id === id ? { ...p, name: formData.name, description: formData.description, icon: formData.icon, color: formData.color, tags, url: formData.url || null, price: formData.price || null, is_available: formData.is_available } : p))
    setEditingId(null)
    startTransition(() => router.refresh())
  }
  ...
  async function handleCreate(formData: typeof EMPTY_FORM) {
    ...
    const res = await fetch('/api/admin/produkte', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...formData, tags, url: formData.url || null, price: formData.price || null }),
    })
    const json = await res.json()
    setCreateLoading(false)
    if (!res.ok) { setCreateError(json.error ?? 'Fehler.'); return }
    setItems(prev => [...prev, json.produkt])
    setShowForm(false)
    startTransition(() => router.refresh())
  }
    ...
    <div className="space-y-3">
      {items.length === 0 && (
        <div className="bg-gray-50 border border-dashed border-gray-200 rounded-2xl p-10 text-center text-gray-400 text-sm">
          Noch keine Produkte vorhanden. Füge dein erstes hinzu.
        </div>
      )}
          ...
          <div key={item.id} className={`bg-white rounded-2xl border transition-opacity ${!item.is_visible ? 'opacity-50 border-gray-100' : isEditing ? 'border-brand/30' : 'border-gray-200'}`}>
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {item.price && <span className="text-xs font-semibold text-brand">{item.price}</span>}
                      {!item.is_available && <span className="text-[10px] font-bold uppercase tracking-wider text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">Coming soon</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 flex-wrap">
                    <SmallToggle checked={item.is_visible} onChange={() => handleToggle(item, 'is_visible')} label="Sichtbar" />
                    <SmallToggle checked={item.is_available} onChange={() => handleToggle(item, 'is_available')} label="Verfügbar" />
            ...
            {isEditing && (
              <div className="border-t border-gray-100 px-5 pb-5 pt-4">
                <ProduktForm
                  initial={{ name: item.name, description: item.description, icon: item.icon, color: item.color, tags: item.tags.join(', '), url: item.url ?? '', price: item.price ?? '', is_available: item.is_available }}
      ...
      {showForm && (
        <div className="bg-white border border-brand/20 rounded-2xl p-6">
          <h3 className="font-bold text-gray-900 text-[15px] mb-4">Neues Produkt</h3>
          <ProduktForm initial={EMPTY_FORM} onSave={handleCreate} onCancel={() => { setShowForm(false); setCreateError(null) }}
            loading={createLoading} error={createError} submitLabel="Produkt erstellen" />
        </div>
      )}

      <p className="text-xs text-gray-400 pt-1">
        <strong className="text-gray-500">Sichtbar</strong> = wird angezeigt &nbsp;·&nbsp;
        <strong className="text-gray-500">Verfügbar</strong> = live (sonst "Coming soon" Badge)
      </p>
    </div>
  )
}
